  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-12">
      <div className="flex items-center justify-between mb-10">
        <h1 className="text-4xl font-bold text-[#f0ede6]" style={{ fontFamily: "'Syne', sans-serif" }}>
          Blog Posts
        </h1>
        <Link
      ...
      {posts.length === 0 ? (
        <div className="border border-[#1f1f1f] rounded-xl p-16 text-center">
          <p className="text-[#8a8a8a] text-sm mb-4" style={{ fontFamily: "'Onest', sans-serif" }}>No posts yet.</p>
          <Link href="/admin/posts/new" className="text-[#00e676] text-sm" style={{ fontFamily: "'Onest', sans-serif" }}>Write your first post →</Link>
        </div>
      ) : (
          ...
          {posts.map((post) => (
            <div key={post.id} className="bg-[#141414] border border-[#1f1f1f] rounded-xl px-6 py-5 flex items-center gap-6 hover:border-[#2a2a2a] transition-colors">
              ...
              <div className="flex items-center gap-3 shrink-0">
                {post.published && (
                  <Link href={`/blog/${post.slug}`} target="_blank" className="text-xs text-[#8a8a8a] hover:text-[#f0ede6] transition-colors" style={{ fontFamily: "'Onest', sans-serif" }}>View ↗</Link>
                )}
                <Link href={`/admin/posts/${post.id}`} className="text-xs text-[#8a8a8a] hover:text-[#00e676] transition-colors px-3 py-1.5 border border-[#1f1f1f] rounded-lg" style={{ fontFamily: "'Onest', sans-serif" }}>Edit</Link>
